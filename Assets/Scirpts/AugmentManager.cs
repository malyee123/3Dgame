...
public class AugmentManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        allAugments = new AugmentData[]
        {
            ...
            new AugmentData { augmentName="물량공세",                 requiredTier=1, type=AugmentType.MassAssault,
                summary="2티어 유닛 최대 20마리 즉시 소환",
                description="즉시 고급 등급(2티어) 유닛을 최대 20마리 소환합니다.\n빈 슬롯이 부족하면 빈 슬롯만큼만 소환됩니다." },
            new AugmentData { augmentName="부러진 검날",              requiredTier=1, type=AugmentType.BrokenBlade,
                summary="모든 유닛 공격력 +8%",
                description="모든 유닛의 공격력이 8% 증가합니다." },
            new AugmentData { augmentName="잊힌 검자루",              requiredTier=1, type=AugmentType.ForgottenHandle,
                summary="모든 유닛 공격속도 +8%",
                description="모든 유닛의 공격속도가 8% 증가합니다." },
            new AugmentData { augmentName="잊혀진 영웅의 검 (세트효과)", requiredTier=1, type=AugmentType.ForgottenSword,
                summary="[세트] 부러진 검날·잊힌 검자루 효과 +30%",
                description="부러진 검날과 잊힌 검자루를 모두 보유 시 선택 가능합니다.\n두 증강의 효과가 각각 30% 증가합니다.\n(공격력 추가 +2.4%, 공격속도 추가 +2.4%)" },
            new AugmentData { augmentName="저주받은 보석",             requiredTier=1, type=AugmentType.CursedGem,
                summary="즉시 +2000골드 / 3웨이브 킬보상 차단",
                description="즉시 2000골드를 획득합니다.\n앞으로 3웨이브 동안 적 처치 시 골드를 획득할 수 없습니다." },
            new AugmentData { augmentName="처음부터 다시",             requiredTier=1, type=AugmentType.StartOver,
                summary="전 유닛 판매 후 판매가의 5배 골드 획득",
                description="보유한 모든 유닛을 즉시 판매합니다.\n판매한 유닛의 판매가 합산의 5배에 해당하는 골드를 획득합니다." },
        };
    ...
    public AugmentData[] GetRandomAugments()
    {
        ...
        {
            if (selectedAugments.Contains(data.type)) continue;
            if (data.requiredTier > unlockedTier) continue;
            if (data.type == AugmentType.ForgottenSword)
            {
                if (!selectedAugments.Contains(AugmentType.BrokenBlade) ||
                    !selectedAugments.Contains(AugmentType.ForgottenHandle))
                    continue;
            }
            pool.Add(data);
        }
    ...
    public void ApplyAugment(AugmentData data)
    {
        ...
        switch (data.type)
        {
            ...
            case AugmentType.BouncingArrow:
                ...
                break;
            case AugmentType.MassAssault:
                SpawnTier2Units(20);
                break;
            case AugmentType.BrokenBlade:
                hasBrokenBlade = true;
                ApplyToAllUnits(8f, 0f);
                break;
            case AugmentType.ForgottenHandle:
                hasForgottenHandle = true;
                ApplyToAllUnits(0f, 8f);
                break;
            case AugmentType.ForgottenSword:
                hasForgottenSword = true;
                ApplyToAllUnits(SWORD_DAMAGE_BONUS, SWORD_SPEED_BONUS);
                break;
            case AugmentType.CursedGem:
                CoinManager.Instance?.AddCoins(2000);
                cursedGemWavesLeft = 4;
                break;
            case AugmentType.StartOver:
                SellAllUnitsWithBonus(5f);
                break;
        }
    ...
    void SpawnTier2Units(int maxCount)
    {
        if (PlayerSpawner.Instance == null) return;
        if (PlayerSpawner.Instance.characterDataList == null) return;
        List<CharacterData> tier2List = new List<CharacterData>();
        foreach (CharacterData cd in PlayerSpawner.Instance.characterDataList)
            if (cd != null && cd.tier == 2) tier2List.Add(cd);
        if (tier2List.Count == 0) return;
        PlayerSpawner.Instance.SyncSlotStateFromScene();
        for (int i = 0; i < maxCount; i++)
        {
            if (PlayerSpawner.Instance.IsFieldFull()) break;
            PlayerSpawner.Instance.SpawnSpecificCharacter(tier2List[Random.Range(0, tier2List.Count)]);
        }
    }

    void SellAllUnitsWithBonus(float bonusMultiplier)
    {
        if (PlayerSpawner.Instance == null) return;
        PlayerAttack[] allUnits = FindObjectsByType<PlayerAttack>(FindObjectsSortMode.None);
        ...
        foreach (PlayerAttack unit in allUnits)
        {
            ...
            totalGold += Mathf.RoundToInt(unit.characterData.sellPrice * sellMult * bonusMultiplier);
        }
        foreach (PlayerAttack unit in allUnits)
        {
            if (unit == null) continue;
        ...
        CoinManager.Instance?.AddCoins(totalGold);
    ...
    public void ApplyAugmentBonusToUnit(PlayerAttack unit)
    {
        ...
        if (selectedAugments.Contains(AugmentType.BrokenBlade))
            unit.ApplyAugmentBonus(8f, 0f);
        if (selectedAugments.Contains(AugmentType.ForgottenHandle))
            unit.ApplyAugmentBonus(0f, 8f);
        if (selectedAugments.Contains(AugmentType.ForgottenSword))
            unit.ApplyAugmentBonus(SWORD_DAMAGE_BONUS, SWORD_SPEED_BONUS);
    }
